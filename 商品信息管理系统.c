/**************************************************************************************
	...
	亮点：1、操作流畅，无需每次按回车键 
		  2、带密码功能，可修改（会加密保存为文件） 
		  3、页面只在一面，不用下拉查看，上部只显示商品信息，提示信息只显示在下部 
		  4、选定功能，直接上下移动选定待批量操作项（删除、修改、搜索、统计）
		  5、搜索功能集精确、模糊为一体，可多级查询
		  6、统计可输出为文件
		  
	缺点：1、页面存在闪屏现象！！暂未找到更好的解决方法
		  2、有些选项不能返回上一层
		  3、修改指定信息时，有些选项无不修改选择
		  4、搜索功能搜索到结果后，无法继续对结果进行修改，删除等操作
		  5、原计划的多层排序功能未实现，原计划的名次和编号“前缀+递增/减数字/字母”“ 递增/减数字/字母+后缀”（递增/减数字/字母  部分空）两项功能未实现
		  6、由于链表不熟，采用了数组的做法，很耗内存，程序已达1.12M 
